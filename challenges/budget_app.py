...
# def output(budget_category):
#     lines = []
#     len_category = len(budget_category.category_name)
#     stars_left = int((half_line_length - len_category / 2))
#     if len_category % 2 == 0:
#         stars_right = stars_left
#     else:
#         stars_right = stars_left + 1
#     lines.append(stars_left * "*" + budget_category.category_name + stars_right * "*")
#     for i, item in enumerate(budget_category.ledger):
#         if i == 0:
#             lines.append("initial amount" + " " * 9 + f"{item['amount']:.2f}")
#         else:
#             num_padding = 7 - len(f"{item['amount']:.2f}")
#             if (len_desc := len(item['description'])) > 23:
#                 description = item['description'][:23]
